
# set enviroment
echo -1 | sudo tee /proc/sys/kernel/perf_event_paranoid > /dev/null
echo 0 | sudo tee /proc/sys/kernel/kptr_restrict > /dev/null
echo 0 | sudo tee /proc/sys/kernel/randomize_va_space > /dev/null
echo 100000 | sudo tee /proc/sys/kernel/perf_event_max_sample_rate > /dev/null
# 39750

# compile program
gcc -O0 -g ./test_perf_hit_count.c -o ./test_perf_hit_count
perf stat -e instructions ./test_perf_hit_count
perf record -e ibs_op// -F max --running-time --data ./test_perf_hit_count

perf script -F +addr,+time,+data_src --ns -i ./perf.data > ./script.log
python3 ./filter.py

# set enviroment
echo 4 | sudo tee /proc/sys/kernel/perf_event_paranoid > /dev/null
echo 1 | sudo tee /proc/sys/kernel/kptr_restrict > /dev/null
echo 2 | sudo tee /proc/sys/kernel/randomize_va_space > /dev/null
